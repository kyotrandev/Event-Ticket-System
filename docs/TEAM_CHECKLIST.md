...
| Dozzle (log UI) | http://localhost:9999 | https://logs.event-ticket-system.kyotran.dev |
...
#### Option A — Dozzle (web UI, khuyến nghị cho team)

**Dozzle** → http://localhost:9999

Real-time log viewer cho tất cả containers. Không cần CLI.

```bash
# Start Dozzle cùng infra
docker compose up -d postgres redis maildev adminer dozzle
```

Tính năng:
- Stream real-time log mọi container
- Filter theo container (chọn `api` hoặc `web`)
- Full-text search trong log stream
- Highlight ERROR/WARN tự động

#### Option B — Docker CLI (terminal)
